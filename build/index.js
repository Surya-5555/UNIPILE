...
//Tools
server.tool("unipile_list_all_accounts", "List all messaging accounts connected to Unipile", {}, async (_input, context) => {
...
server.tool("unipile_get_gmail_emails", "Fetch recent emails from connected Gmail account (auto-detects account)", {
    ...
    try {
        ...
        const gmailAcc = allAccounts.find((acc) => acc.email?.includes("@gmail.com") && acc.type === "GOOGLE_OAUTH");
        if (!gmailAcc) {
            return {
